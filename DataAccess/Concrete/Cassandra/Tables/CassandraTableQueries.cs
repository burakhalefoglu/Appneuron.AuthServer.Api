namespace DataAccess.Concrete.Cassandra.Tables
{
    public static class CassandraTableQueries
    {
        ...
        public static string User => "CREATE TABLE IF NOT EXISTS authdatabase.users(id bigint, name text, email text, record_date timestamp, update_contact_date timestamp, password_salt blob, password_hash blob, reset_password_token text, reset_password_expires timestamp, status boolean, PRIMARY KEY((email, status)))";
