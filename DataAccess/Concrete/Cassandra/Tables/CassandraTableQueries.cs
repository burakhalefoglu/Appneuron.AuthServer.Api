namespace DataAccess.Concrete.Cassandra.Tables
{
    public static class CassandraTableQueries
    {
        ...
        public static string OperationClaim => "CREATE TABLE IF NOT EXISTS AuthDatabase.operation_claims(id bigint, name text, alias test, description text, status boolean, PRIMARY KEY(id))";
        public static string Translate => "CREATE TABLE IF NOT EXISTS AuthDatabase.translates(id bigint, code test, value text, status boolean, PRIMARY KEY(id))";
        public static string User => "CREATE TABLE IF NOT EXISTS AuthDatabase.users(id bigint, name text, email text, record_date date, update_contact_date date, password_salt blob, password_hash blob, reset_password_token text, reset_password_expires text, status boolean, PRIMARY KEY(id))";
        public static string UserClaim => "CREATE TABLE IF NOT EXISTS AuthDatabase.user_claims(id bigint, users_ıd bigint, claim_id bigint, status boolean, PRIMARY KEY(id))";
        public static string UserGroup => "CREATE TABLE IF NOT EXISTS AuthDatabase.user_groups(id bigint, users_ıd bigint, group_id bigint, status boolean, PRIMARY KEY(id))";
    }
}